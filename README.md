...
**Версия растёт автоматически:** `versionCode`/`versionName` = `0.0.<номер
пайплайна CI>` (видно во вкладке «О программе»), отсчёт с 0.0.17. Каждая сборка
ветки `android` заливает APK/AAB в Package Registry и создаёт **GitLab Release**
с тегом `v0.0.<номер>` — качать удобнее из раздела **Releases**, а не из артефактов.
