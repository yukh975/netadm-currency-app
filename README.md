...
- Ежедневное уведомление-сводка (WorkManager): по строке на валюту,
  «1 EUR = 90.50 RUB» (курс к основной). Время присылки выбирается в настройках
  (по умолчанию 17:00 МСК); тап по уведомлению открывает приложение со сводкой.
- Сводка по запросу: кнопка «🔔 Сводка курсов» на экране конвертации — те же
  курсы избранных валют к основной, не дожидаясь уведомления. Данные при этом
  обновляются принудительно (всегда свежие).
...
`0.2.0`); поднимайте её при выпуске новой версии. `versionCode` растёт
